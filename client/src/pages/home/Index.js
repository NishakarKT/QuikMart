...
import { LocationSearching, LightMode, DarkMode, Category } from '@mui/icons-material';
// contexts
...
const Index = () => {
  ...
  const { user, mode, handleMode, category, setCategory } = useContext(AppContext);
  ...
  const getProducts = useCallback(() => {
    (async () => {
      try {
        ...
        axios.get(PRODUCT_GET_PRODUCTS_BY_LOCATION_ENDPOINT, { params: query })
          .then(res => {
  ...
  useEffect(() => {
    if (user) {
      try {
        axios.get(PRODUCTS_GET_WISHLIST_ENDPOINT, { params: { userId: user._id } })
          .then(res => {
      ...
      try {
        axios.get(PRODUCTS_GET_CART_ENDPOINT, { params: { userId: user._id } })
          .then(res => {
            const productIds = res.data.data;
      ...
      try {
        axios.get(PRODUCT_GET_ORDERS_ENDPOINT, { params: { from: user._id } })
          .then(res => {
  ...
  return (
    <Container disableGutters className={classes.container}>
      ...
      <HomeContext.Provider value={{ product, setProduct, products, setProducts, locationRange, category, setLocationRange, handleLocationRange, wishlist, setWishlist, cart, setCart, orders, setOrders }}>
        <Navbar />
        <Routes>
          <Route exact path="/search/:query" element={<Search />} />
          <Route exact path="/orders" element={user ? <Orders /> : <Navigate to={"/auth"} />} />
          <Route exact path="/cart" element={user ? <Cart /> : <Navigate to={"/auth"} />} />
          <Route exact path="wishlist" element={user ? <WishList /> : <Navigate to={"/auth"} />} />
          <Route exact path="/profile" element={user ? <Profile /> : <Navigate to={"/auth"} />} />
