...
export const editProduct = async (req, res) => {
    ...
    if (updates.deletedFiles?.length)
        updates.deletedFiles.forEach(deletedFile => fs.unlink("media/" + deletedFile, err => { console.log(err) }));
...
export const newProducts = async (req, res) => {
    ...
    try {
        Product.collection.insert(data, (err, docs) => {
            if (err)
                throw "couldn't create all the products";
            else
                res.status(201).send("created products");
        });
    } catch (err) { res.status(500).send({ message: err.message }); };
};
...
export const newOrders = async (req, res) => {
    ...
    try {
        ...
        if (!result)
            throw "couldn't create all the orders";
        else
            res.status(201).send("created orders");
    } catch (err) { res.status(500).send({ message: err.message }); };
}